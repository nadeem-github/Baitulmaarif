...
@Component({
  selector: 'app-islahi-parche',
  templateUrl: './islahi-parche.component.html',
  styleUrls: ['./islahi-parche.component.scss']
})
export class IslahiParcheComponent implements OnInit {
  ...
  private getMolanaBayanList(): void {
    ...
    this.shortClipService.specialCollectionList(this.ShortClipModal).subscribe(
      (response: any) => {
        this.loading = false;
        if (response.Status) {
          this.dataDuayen = response.Data.filter((item: any) => item.Category === 'Islami Perche');
