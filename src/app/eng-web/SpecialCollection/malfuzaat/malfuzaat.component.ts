...
@Component({
  selector: 'app-malfuzaat',
  templateUrl: './malfuzaat.component.html',
  styleUrls: ['./malfuzaat.component.scss']
})
export class MalfuzaatComponent implements OnInit {
  ...
  private getMolanaBayanList(): void {
    ...
    this.shortClipService.specialCollectionList(this.ShortClipModal).subscribe(
      (response: any) => {
        this.loading = false;
        if (response.Status) {
          this.dataDuayen = response.Data.filter((item: any) => item.Category === 'Malfoozat');
